...
// 	    $show       = $Page->show();// 分页显示输出
	    if($arts!=null){
	        ...
	        foreach ($arts as $art){
	            //将时间戳转化日期类型
	            $arts[$i]['new_time'] = date("Y-m-d",$art['new_time']);
	            //查找它属于那个分类
	            $arts[$i]['cid'] = $Cate->field('mname')->where('id='.$art['cid'])->find()['mname'];
	            //将过滤html标签
	            $arts[$i]['content'] = strip_tags(html_entity_decode($art['content']));
	            $i++;
	        }
	        foreach ($arts as $art){
	           $str=<<<cont
          ...
          {$art[id]}</td>
          <td width="10%"><img src="{$art['image']}" alt="" width="70" height="50" /></td>
          <td>{$art['title']}</td>
          <td>{$art['desc']}</td>
          <td>{$art['content']}</td>
          <td>{$art['keyword']}</td>
          <td>{$art['cid']}</td>
          <td>{$art['admin_name']}</td>
          <td>{$art['check_count']}</td>
          <td>{$art['new_time']}</td>
          <td><div class="button-group"> <a class="button border-main" href="{:U('Admin/article/artedit',array('id'=>$art[id]))}"><span class="icon-edit"></span> 修改</a> <a class="button border-red" href="javascript:void(0)" onclick="return del({$art['id']})"><span class="icon-trash-o"></span> 删除</a> </div></td>
               </tr>
cont;
        echo $str;
	        }
